...
class UserQueryView(APIView):
    ...
    async def call_mcp_api(self, message: str, user_id: str, token: str, action_type: str = None,
        ...
        try:
            # 访问 MCP API 地址
            # 注意：这里设置了合适的 MCP API URL，但需要确保这个设置在 settings.py 中存在
            mcp_api_url = "http://localhost:8000/api/mcp/process/"  # 默认值作为示例

            # 使用设置中的URL如果有定义
            if hasattr(settings, 'MCP_API_URL'):
